...
class TimePatternConfiguratorList(BaseComponent):
    ...
    def _init_state(self):
        ...
        self._session_state['loaded_time_pattern_filenames'] = set()
    ...
    def _load(self, filename: str) -> None:
        ...
        if filename in self._session_state['loaded_time_pattern_filenames']:
            default_notifier(
        ...
        try:
            time_pattern_data = load_time_pattern(filename)
            time_pattern = models.TimePatternConfig.model_validate(
                obj=time_pattern_data
            )
        except ContentManagementError as e:
            default_notifier(
                message=f'Failed to load time pattern: {e}',
                level=NotificationLevel.ERROR
            )
            return
        except ValidationError as e:
            default_notifier(
                message=prettify_errors(e.errors()),
                level=NotificationLevel.ERROR
            )
            return
        ...
        self._session_state['loaded_time_pattern_filenames'].add(filename)
    ...
    def _delete(self, id: int) -> None:
        ...
        if time_pattern.is_saved():
            self._session_state['loaded_time_pattern_filenames'].remove(
